...
func TestDeleteDataSourceNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest("DELETE", "/datasources/not-found", nil)
...
func TestDeleteDataSource(t *testing.T) {
	...
	preReq, _ := http.NewRequest("GET", "/datasources/to-be-deleted", nil)
	router.ServeHTTP(pre, preReq)

	assert.Equal(t, http.StatusOK, pre.Code)

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("DELETE", "/datasources/to-be-deleted", nil)
	router.ServeHTTP(w, req)

	assert.Equal(t, http.StatusOK, w.Code)

	post := httptest.NewRecorder()
	postReq, _ := http.NewRequest("GET", "/datasources/to-be-deleted", nil)
